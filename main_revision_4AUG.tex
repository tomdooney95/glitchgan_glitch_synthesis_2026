...
DeepExtractor achieved low time-domain mismatch against the maximum-likelihood templates from standard parameter estimation workflows (see FIG. 11 in~\cite{DeepExtractor_paper}), where all single-detector signals with matched-filter signal-to-noise ratio $\rho_{MF}>13.5$ are reconstructed with under 10\% mismatch against maximum-likelihood templates.
...
Because per-class accuracy is a binomial proportion (the fraction of $n$ trials classified into the correct class), we compute intervals using the Wilson score method~\cite{Wilson01061927}, which---unlike the standard normal-approximation interval---remains bounded within $[0,1]$ and maintains accurate coverage even for small sample sizes or accuracies near the extremes (e.g. 100\%, as with the Low Frequency Burst class). 
...
\subsubsection{Downstream Classification between real and synthetic glitches\label{sec:downstream_classification_methods}}
...
To evaluate whether GlitchGAN-generated glitches are distinguishable from real DeepExtractor reconstructions, we train a binary classifier to discriminate between the two populations. To this end, we train a new GlitchGAN model on the same dataset, but holding out 10% of the data for the downstream classification, stratifying the split by class. This amounts to 28{,}360 real training samples across the seven glitch classes, which we rebalance to 5{,}000 samples per class (35{,}000 total) following the same procedure used for the main model, and 3{,}151 held-out real samples that are never seen during training of this holdout GlitchGAN. This holdout model is trained separately from, and does not replace, the main GlitchGAN model reported throughout this work.

Once training of the holdout model is complete, we use its generator to produce one synthetic glitch for each of the 3{,}151 held-out real glitches, conditioned on the same class label, giving a balanced evaluation pool of 6{,}302 samples (3{,}151 real, 3{,}151 generated). Critically, neither the real samples in this pool nor the samples used to generate their synthetic counterparts were seen during the holdout model's training, so any inability of a classifier to separate the two populations reflects genuine physical similarity rather than memorization. We evaluate using the holdout generator's epoch-210 checkpoint, selected via the same criteria (qualitative inspection of sample realism and generator loss stability) used to select the main model's reported checkpoint (Section~\ref{sec:training_procedure}), rather than the final epoch of training; Section~\ref{sec:downstream_classification_results} additionally reports results across several other training checkpoints as a robustness check.

We use a 1D convolutional neural network (CNN) with three convolutional layers followed by two fully-connected layers to perform the classification, deliberately kept simpler than and architecturally independent of GlitchGAN's own discriminator, so that it constitutes a genuinely independent check rather than a re-run of the adversarial training objective. The full architecture is given in Table~\ref{tab:real_fake_clf_arch}. Each convolutional layer uses a kernel size of 16 and a stride of 4, progressively downsampling the 8192-sample input while increasing the channel depth from 32 to 128, followed by global average pooling, a 64-unit dense layer with dropout ($p=0.3$), and a single sigmoid output unit. The network has approximately 173k trainable parameters and is trained with the Adam optimizer and a binary cross-entropy loss.

The balanced real/generated pool is split 70/10/20 into training, validation, and test sets, stratified jointly by class and by real/fake domain -- giving 4{,}412 training samples, 632 validation samples, and 1{,}258 test samples, with real and generated samples represented in equal number within every class and every split (Table~\ref{tab:clf_split_breakdown}). Prior to classification, both real and generated samples are scaled identically via the same per-sample min-max and mean-subtraction procedure used to prepare GlitchGAN's training data, so the classifier cannot exploit a trivial normalization mismatch between the two domains. As an additional check on the strength of the classification signal, we also train a simple linear baseline -- logistic regression fit directly on the raw waveform, with its L2 regularization strength selected via the validation split -- alongside the CNN, to establish how much of any discriminating information is accessible to a model with no capacity for nonlinear feature extraction. We report test accuracy together with its 95% Wilson score confidence interval, computed using the same procedure described in Section~\ref{sec:gspy_classification_methodology}, for both classifiers.
...
% The corresponding real vs synthetic class-wise UMAP projections using the same embedding are shown in Figure~\ref{fig:umap_per_class_rotated}.}
...
% Three-dimensional UMAP embeddings for each glitch class, comparing real (blue) and GlitchGAN-generated (green) samples for the same UMAP embedding shown in Figure~\ref{fig:umap_two_views}.
...
\caption[Per-class UMAP embeddings of real and synthetic glitches]{
Two-dimensional UMAP embeddings for each glitch class, comparing real (blue) and GlitchGAN-generated (green) samples for the same UMAP embedding shown in Figure~\ref{fig:umap_two_views}.
Each panel corresponds to one glitch type, showing the same 800 real and 800 generated samples per class plotted in Figure~\ref{fig:umap_two_views}.
The close overlap between real and synthetic distributions across most classes demonstrates that GlitchGAN effectively learns class-specific morphological features, though some variation remains for more complex glitch morphologies.}
\label{fig:umap_per_class_rotated}
\end{figure*}

\subsubsection{Downstream Classification between real and synthetic glitches\label{sec:downstream_classification_results}}

The classifier distinguishes real from generated glitches with high accuracy across all four holdout-generator checkpoints tested (epochs 210, 300, 400, and 500), ranging from 95.7\% to 98.1\% on the held-out test split, all well outside the 50\% chance level (Table~\ref{tab:real_fake_epoch_sweep}). Discriminability is not monotonic in training epoch: the checkpoint selected using the same qualitative and UMAP-based criteria applied to the main model (epoch 210) achieves the lowest CNN accuracy (95.7\%, 95\% CI [94.4, 96.7]\%), statistically tied with epoch 500 (95.9\%, 95\% CI [94.7, 96.9]\%), and both significantly lower than epochs 300 and 400 (98.0\% and 98.1\% respectively; non-overlapping 95\% CIs). This pattern is consistent with the well-documented instability of adversarial training and reinforces the rationale for selecting the reported checkpoint via qualitative and structural criteria rather than the final epoch of training (Section~\ref{sec:downstream_classification_methods}).

A linear baseline (logistic regression fit directly on the raw waveform, with its L2 regularization strength selected via the validation split) is consistently far less capable of this discrimination, ranging from 69.2\% to 77.6\% accuracy across the same checkpoints and always substantially below the corresponding CNN accuracy (Table~\ref{tab:real_fake_epoch_sweep}). Notably, the linear baseline's accuracy decreases monotonically with training epoch, consistent with the generator's coarse statistical properties -- such as its amplitude and dynamic-range variability -- progressively converging toward those of real data as training proceeds. The CNN's accuracy does not follow this trend, indicating it is sensitive to an additional, more complex source of discriminating information whose strength varies non-monotonically with training epoch, consistent with the broader instability of adversarial training already reflected in the checkpoint-selection process for the main model. Comparing summary statistics between the two populations at the epoch-210 checkpoint shows that real glitches exhibit greater sample-to-sample variability in their amplitude and dynamic-range characteristics (row-wise minimum/maximum standard deviation: 0.367 for real vs. 0.270 for generated) than generated glitches, a difference plausibly responsible for the shrinking linear-baseline signal, though it does not fully account for the CNN's higher and non-monotonic accuracy -- the precise nature of that additional nonlinear signal remains uncharacterized. This is consistent with the strong class-level and structural agreement between real and synthetic glitches already captured by Gravity Spy classification and the UMAP embeddings (Sections~\ref{sec:GlitchGAN_gspy_eval} and~\ref{sec:glitchgan_umap}): a classifier trained specifically to discriminate between the two populations can still detect subtler, lower-level statistical differences at the raw-waveform level that these coarser methods do not resolve. This mirrors a broader finding in the generative modelling literature: purpose-built classifiers routinely detect GAN-generated content with high accuracy even when it is otherwise realistic by coarser measures~\cite{wang2020cnngenerated}. We interpret this result not as evidence that GlitchGAN's synthetic glitches are morphologically dissimilar from real ones, but as a caveat on the resolution at which physical consistency holds: GlitchGAN reproduces class-level morphology and gross structural similarity faithfully, but synthetic glitches remain distinguishable from real ones under sufficiently targeted, fine-grained statistical scrutiny.
% \FloatBarrier
...
\bottomrule
\end{tabular}
\caption{Real-vs-generated classifier accuracy across four holdout-generator training checkpoints, for both the CNN and the linear (logistic regression) baseline. Epochs 210 and 500 show the lowest CNN-detectable difference between real and generated glitches, with non-overlapping confidence intervals relative to epochs 300 and 400 -- a non-monotonic pattern consistent with the training instability already observed qualitatively when selecting the main model's reported checkpoint (Section~\ref{sec:downstream_classification_methods}).}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/lfb_real_vs_generated.pdf}
...
\caption{Architecture and hyperparameters of cDVGAN featured in GlitchGAN, for the 8192-sample glitch waveforms used here (10.2M parameters total). The model includes a base discriminator, derivative (DV) discriminator, and generator. ``Stride'' for the generator's Upsample+Conv. blocks is the Conv1D stride (always 1); the $\times$2 upsampling itself is a separate nearest-neighbour \texttt{UpSampling1D} step before each convolution, not a learned strided transposed convolution. ``Class embedding'' in the discriminators is an \texttt{Embedding} lookup over the argmax class index, not a dense projection of the one-hot class vector.}
...
\end{tabular}}
\captionsetup{width=0.5\textwidth}
\caption{Architecture of the 1D CNN classifier used to discriminate between real and GlitchGAN-generated glitches in the downstream classification test. The network takes an 8192-sample time series as input and outputs a single scalar in $[0,1]$, trained with a binary cross-entropy loss to predict real (1) vs.\ generated (0). Total trainable parameters: 172{,}897.}
