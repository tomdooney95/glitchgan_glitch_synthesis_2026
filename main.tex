...
\title{Realistic Time-DomainSynthesis of Gravitational-Wave Detector Glitches using Class-Conditional Derivative Generative Adversarial Networks}
...
A possible approach is to model glitches mathematically, given \textit{a priori} knowledge of their production mechanism \cite{Tolley:2023umc}. 
...
However, such approaches either require prior knowledge of the glitch's physical origin or are limited to a restricted parametric form, making them difficult to scale across the full diversity of observed morphologies.
...
The \textit{gengli} glitch generator~\cite{GENGLI, GENGLI_2} demonstrated that a GAN trained on Blip glitches extracted from detector backgrounds using BayesWave~\cite{BayesWave} can successfully learn their underlying time-domain distribution, providing a useful framework for simulations and MDCs.
McGANn~\cite{McGinn_2021} extended this to a conditional GAN capable of synthesizing five distinct waveform classes in one model analogous to GW bursts, with class interpolation enabling hybrid morphologies.
...
\textit{Blip} glitches have a characteristic morphology of a symmetric teardrop shape in time-frequency in the range $[30, 300]\,$Hz with short-durations, $\sim 0.04\,$s. They appear in both Hanford and Livingston, as well as Virgo and GEO $600$ \cite{Cabero:2019orq}. 
Due to their abundance and form, these glitches hinder both the unmodeled burst and modelled CBC searches, with particular emphasis on compact binaries with large total mass, highly asymmetric component masses, and spins anti-aligned with the orbital angular momentum  \cite{abbott2016characterization}.
...
\textit{Whistles} have a characteristic V, U or W shape at higher frequencies with typical durations $\sim 0.25\,$s.m They are caused when radio-frequency signals beat with the voltage-controlled oscillators \cite{Nuttall:2015dqa}.
...
We evaluate both the DeepExtractor training data and GlitchGAN-generated glitches using two complementary approaches: classification with the Gravity Spy classifier and visualisation of low-dimensional embeddings via t-SNE and UMAP. 
...
Since Gravity Spy classifies glitches surrounded by detector background noise (i.e. $g(t) + n(t)$), each DeepExtractor reconstruction ($\hat{g(t)}$) is injected into a segment of `quiet' Hanford detector noise from O3 prior to reclassification (representing $n(t)$). 
...
Tables~\ref{tab:de_summary} and~\ref{tab:de_per_class} summarize the results of classifying 700 DeepeExtractor reconstructions (100 per class) with Gravity Spy.
...
To quantify synthetic glitch realism in a regime relevant to GW data analysis, we injected generated glitches into quiet LIGO Hanford O3 background noise at SNR = 50 and classified them using the Gravity Spy classifier.
...
While this may appear low, the misclassifications are consistent with known limitations of Gravity Spy and the use of a fixed injected SNR of 50. 
Most misclassifications occur in the Koi Fish and Scattered Light classes. 
While only 20\% of the generated Koi Fish samples were correctly classified, the majority of the remaining samples were misidentified as Blip, Blip Low Frequency, or Tomte.
...
Moreover, increasing the injected SNR of the generated Koi Fish samples to 150 raises the classification accuracy to 79\% with an average confidence of $\sim$95\%, further illustrating the dependence of Gravity Spy classifications on the injected SNR.
...
A comparable SNR dependence is observed for the Whistle class.
...
This was also observed for DeepExtractor's original reconstructions of both Fast Scattering andScattered Light shown in Figure~\ref{fig:de_confusion}.
...
Figure~\ref{fig:umap_two_views} shows two complementary perspectives of this embedding space, computed from 500 samples per class.
...
Figure~\ref{fig:hybrid_glitch_mixing} shows waveform examples from both mixing strategies, demonstrating intermediate morphologies.
...
The UMAP projection of mixed samples (Figure~\ref{fig:simplex_uniform_umap}) confirms that these hybrid samples smoothly interpolate between pure-class point clouds, with uniform mixing exploring the broadest region of the latent space.
...
We further evaluated the DiT model as a comparison; the full UMAP and Gravity Spy results are shown in Appendix~\ref{app:dit_evaluation} (Figures~\ref{fig:dit_umap} and~\ref{fig:dit_confusion}).
...
Figure~\ref{fig:low_freq_burst_examples} illustrates how time-domain differences can be masked in magnitude-only spectrogram representations.
...
Partial failure was observed for the Whistle class, possibly due to insufficient regularization or architectural constraints on receptive field size. For example, smaller kernel sizes in generator and discriminator  layers may be needed to capture the high-frequency content of Whistle glitches (please see Appendix~\ref{sec:cDVGAN_Architecture} for details).
...
Finally, we note that no hyperparameter optimization was performed for this specific dataset; the model is evaluated ``out-of-the-box'' from previous work, representing a conservative assessment of GlitchGAN's potential.
...
\item
\textbf{Training data augmentation:} Supplementing limited real glitch samples for machine learning algorithms
...
Architecture improvements may include the incorporation ofattention mechanisms, dilated convolutions for multi-scale feature extraction, or architecture search could improve performance.
...
Glitch characterization is a challenging task in GW data analysis. It would be beneficial to develop more sophisticated validation metrics that explicitly evaluate phase consistency, not just magnitude-level morphology, to ensure synthetic glitches are suitable for detector simulations.
...
We have demonstrated that class-conditional deep generative models can effectively synthesize realistic GW detector glitches directly in the time domain by using the cDVGAN architecture.
...
For GlitchGAN's generated glitches, Gravity Spy classified most samples correctly.
...
For instance, Scattered Light glitches were often confused with Fast Scattering, while Koi Fish are sometimes misclassified as Blip or Tomte.
These overlaps are expected, as the classes share similar time-frequency signatures.
This is confirmed by increasing the injected SNR to 150, where 79\% of generated Koi Fish samples are correctly classified.
...
For this class, synthetic samples formed a partially distinct cluster from the real data, suggesting that cDVGAN did not fully capture the fine-scale, high-frequency structure characteristic of these glitches.
...
We observed that even a less robust diffusion-based model, which produces unrealistic Low Frequency Burst glitches in the time domain, can still yield correct Gravity Spy labels with high confidence.
...
Such findings underscore the limitations of magnitude-only training that discards phase information—an essential feature for accurately capturing glitch morphology.
They further highlight the importance of preserving phase information and integrating unsupervised analyses to complement and strengthen glitch characterization efforts.
...
As GW detector networks mature and sensitivity improves, the ability to generate realistic instrumental noise features will become increasingly valuable for characterization and analysis algorithm development.
...
Figures~\ref{fig:dit_confusion} and~\ref{fig:dit_umap} show the full UMAP and Gravity Spy evaluation results for the DiT comparison model discussed in Section~\ref{sec:results}.
